...
var WpImaGeoMapEditorForm = function()
{
	...
	/**
	 * マーカーが未選択状態になった時に呼び出す事で、
	 * 全ての要素を空文字に設定し、入力を読み取り専用にします。
	 */
...
var WpImaGeoMapEditor = function()
{
	...
	this.addMarker = function()
	{
		...
		if( form.isReadExif() )
		{
			...
			jQuery.getJSON( target, function( exif )
				{
...
var WpImaGeoMapViewer = function()
{
	...
	this.initialize = function()
	{
		...
		jQuery( "div.imageomap" ).each( function()
		{
			var f = window[ "imageomap_get_" + id ];
			if( f != undefined )
			{
				var info = f();
				jQuery( this ).append( "<div id=\"imageomap_canvas_" + id + "\" class=\"map\" style=\"width:" + info.width + ";height:" + info.height + "\"></div>" + html );
				maps[ id ] = new WpImaGeoMap( { id: id, div: jQuery( this ), map: info.map, markers: info.markers, line: info.line } );
		...
		if( id > 0 )
		{
			jQuery( "img.imagedetail" ).tooltip( { effect: 'slide' } );
		}
	};
};
